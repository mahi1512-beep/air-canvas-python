...
drawing_points = []  # Store last 5 points for smoother drawing
brush_thickness = 10
eraser_thickness = 50
eraser_mode = False
            ...
            for hand_landmarks in result.multi_hand_landmarks:
                ...
                if lm_list:
                    ...
                    for landmark in lm_list:
                        if landmark[0] == 8: #index finger id
                            index_finger_tip = (landmark[1], landmark[2])
                        if landmark[0] == 12: #middle finger id
                            middle_finger = (landmark[1], landmark[2])
                    if index_finger_tip is not None and middle_finger is not None:
                        ...
                        pinky_tip = None
                        for landmark in lm_list:
                            if landmark[0] == 4:  # Thumb tip
                                thumb_tip = (landmark[1], landmark[2])
                            if landmark[0] == 20: # pinky tip
                                pinky_tip = (landmark[1], landmark[2])
                        if thumb_tip is not None and pinky_tip is not None:
                            fist_gesture = abs(thumb_tip[0] - pinky_tip[0]) < 50 and abs(thumb_tip[1] - pinky_tip[1])<50
                            if fist_gesture:
                                canvas = np.zeros_like(frame)
                        
                        fingers_up = abs(index_finger_tip[1] - middle_finger[1]) < 60
                        ...
                        # DRAWING MODE (Index Finger Only)
                            ...
                            # Draw curve based on the last 5 points
                            if eraser_mode:
                                for i in range(1, len(drawing_points)):
                                    cv2.line(canvas, drawing_points[i - 1], drawing_points[i], (0, 0, 0), eraser_thickness)
                            else:
                                for i in range(1, len(drawing_points)):
